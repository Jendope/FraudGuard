...
Supported models
----------------
* ``qwen3.5-plus``, ``qwen-plus``, ``qwen-turbo``, ``qwen-max``  — Qwen via DashScope (default)
* ``glm5/glm5-llm``                            — GLM-5 via BigModel (zai-sdk)
* ``deepseek-v3``, ``deepseek-r1``             — DeepSeek via DashScope
* Any Hugging Face hub path                    — HFPipelineLLMClient fallback
...
from .llm_client import DeepSeekLLMClient, GLM5LLMClient, HFPipelineLLMClient, QwenLLMClient
...
_REGISTRY: dict[str, Type] = {
    # Qwen models via DashScope (primary — rotate freely for free-tier quota)
    "qwen3.5-plus": QwenLLMClient,
    "qwen-plus": QwenLLMClient,
    "qwen-turbo": QwenLLMClient,
    "qwen-max": QwenLLMClient,
    "qwen-long": QwenLLMClient,
    # GLM-5 via BigModel / ZhipuAI SDK (alternative)
    "glm5/glm5-llm": GLM5LLMClient,
    "glm-5": GLM5LLMClient,
    # DeepSeek via DashScope
    "deepseek-v3": DeepSeekLLMClient,
    "deepseek-r1": DeepSeekLLMClient,
...
def get_llm_client(model_name: str | None = None):
    ...
    model_name:
        ...
        environment variable is used (default ``"qwen-plus"``).
    ...
    # Fallback: treat any unknown slash-delimited string as a Hugging Face
    # hub path and attempt to load it with the generic HFPipelineLLMClient.
    if "/" in model_name:
        logger.warning(
    ...
    raise ValueError(
        ...
        "Register it in backend/modules/llm_factory.py or provide a full "
